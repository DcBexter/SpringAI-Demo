  ...
  loadConversation(conversationId: string): void {
    ...
    this.chatService.getConversationMemory(conversationId).subscribe({
      next: (memory) => {
        ...
        // Load conversation-specific model if available
        // The backend will return the model associated with this conversation
        // For now, we rely on the backend to handle per-conversation model persistence
      },
      error: (error) => {
